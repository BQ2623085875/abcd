 ...
 * @Description : 进港交接
 */
public class EnterPortHandoverDialog extends BaseDialog {
    ...
    private TextView mTvContainerCount ,//货物总斗数
            ...
            mTvTransWarehousingTime ;//拉入库房时间
    private SignView mSignContent ;//签字
    private ImageView mIvSignContent ;
    private CheckBox mCbHaveTaskBag ,//是否有任务袋子
            mCbHaveTask ;//任务指派单
    private boolean IsEditor ;

    private HandoverData mHandoverBean ;

    private static EnterPortHandoverDialog alertDialog ;

    public static EnterPortHandoverDialog getInstance(Context mContext) {
        if(alertDialog == null)
            alertDialog = new EnterPortHandoverDialog(mContext);
        return new EnterPortHandoverDialog(mContext);
    }

    private EnterPortHandoverDialog(Context mContext) {
    ...
    private EnterPortHandoverDialog builder() {
        // 获取Dialog布局
        View view = LayoutInflater.from(mContext).inflate(R.layout.dialog_enter_port_handover, null);
        ...
        mTvTransWarehousingTime = view.findViewById(R.id.mTvTransWarehousingTime);
        mSignContent = view.findViewById(R.id.mSignContent);
        mIvSignContent = view.findViewById(R.id.mIvSignContent);
        mCbHaveTaskBag = view.findViewById(R.id.mCbHaveTaskBag);
        mCbHaveTask = view.findViewById(R.id.mCbHaveTask);
        ...
        public void onClick(View view) {
            switch (view.getId()){
                ...
                case R.id.mTvTransportTime:
                    ...
                    break;
                case R.id.mTvTransWarehousingTime:
                    // 入库时间
                    TimerPickerUtils.showTimePickerView(mContext, !TextUtils.isEmpty(mHandoverBean.getHandoverTime()) ? mHandoverBean.getHandoverTime() : CommonUtils.getTime(new Date()), new OnTimeSelectListener() {
                        @Override
                        public void onTimeSelect(Date date, View v) {
                            mHandoverBean.setHandoverTime(CommonUtils.getTime(date));
                            mTvTransWarehousingTime.setText(CommonUtils.getTime(date));
                        }
                    }).show();
                    break;
            }
     ...
     * @return
     */
    public EnterPortHandoverDialog setData(HandoverData handoverBean , boolean IsEditor){
    ...
    private void initInfo() {
        ...
        mTvTransWarehousingTime.setClickable(IsEditor ? true : false);
        mCbHaveTaskBag.setClickable(IsEditor ? true : false);
        ...
        mCbHaveTaskBag.setChecked(mHandoverBean.getHasBag() == 1);
        mCbHaveTask.setChecked(mHandoverBean.getInstructor() == 1);
        mTvTransportTime.setText(mHandoverBean.getDeliveryTime());
        mTvTransWarehousingTime.setText(mHandoverBean.getHandoverTime());
        ...
        if(IsEditor) {
            ...
            mTvTransWarehousingTime.setOnClickListener(new OnItemClickListener());
        }
    }
     ...
     * @return
     */
    public EnterPortHandoverDialog setCancelable(boolean cancel) {
     ...
     * @return
     */
    public EnterPortHandoverDialog setConfirmButton(final HandoverConfirmListener listener) {
        mBtnConfirm.setOnClickListener(new OnClickListener() {
            ...
            public void onClick(View v) {
                ...
                if(TextUtils.isEmpty(mTvTransWarehousingTime.getText().toString())){
                    ToastUtils.showToast(R.string.toast_trans_warehousing_time_null);
                    return;
                }


                mHandoverBean.setHasBag(mCbHaveTaskBag.isChecked() ? 1 : 0);
                mHandoverBean.setInstructor(mCbHaveTask.isChecked() ? 1 : 0);
    ...
    private void saveHandoverData(HandoverConfirmListener listener){
        ...
        }, JsonUtils.parseBeanToJson(mHandoverBean) , true);
    }
     ...
     * @return
     */
    public EnterPortHandoverDialog setCancelButton() {
     ...
     * @return
     */
    public EnterPortHandoverDialog setCancelButton(final OnClickListener listener) {
        mBtnCancel.setOnClickListener(new OnClickListener() {
            @Override
            public void onClick(View v) {
                mDialog.dismiss();
